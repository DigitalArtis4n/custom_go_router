...
typedef NavigatorBuilder = Widget Function(GlobalKey<NavigatorState> navigatorKey, ShellRouteMatch match, RouteMatchList matchList,
...
abstract class RouteBase with Diagnosticable {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    if (parentNavigatorKey != null) {
      properties.add(DiagnosticsProperty<GlobalKey<NavigatorState>>('parentNavKey', parentNavigatorKey));
    }
  }
}
...
class GoRoute extends RouteBase {
  ...
  RegExpMatch? matchPatternAsPrefix(String loc) {
    return _pathRE.matchAsPrefix('/$loc') as RegExpMatch? ?? _pathRE.matchAsPrefix(loc) as RegExpMatch?;
  }

  /// Extract the path parameters from a match.
  Map<String, String> extractPathParams(RegExpMatch match) => extractPathParameters(pathParameters, match);
...
abstract class ShellRouteBase extends RouteBase {
  ...
  static void _debugCheckSubRouteParentNavigatorKeys(List<RouteBase> subRoutes, GlobalKey<NavigatorState> navigatorKey) {
    for (final RouteBase route in subRoutes) {
      assert(route.parentNavigatorKey == null || route.parentNavigatorKey == navigatorKey,
  ...
  Widget? buildWidget(BuildContext context, GoRouterState state, ShellRouteContext shellRouteContext);
  ...
  Page<dynamic>? buildPage(BuildContext context, GoRouterState state, ShellRouteContext shellRouteContext);
...
class ShellRouteContext {
  ...
  Widget _buildNavigatorForCurrentRoute(List<NavigatorObserver>? observers, String? restorationScopeId) {
    return navigatorBuilder(navigatorKey, match, routeMatchList, observers, restorationScopeId);
  }
}
...
class ShellRoute extends ShellRouteBase {
  ...
  Widget? buildWidget(BuildContext context, GoRouterState state, ShellRouteContext shellRouteContext) {
    if (builder != null) {
      final Widget navigator = shellRouteContext._buildNavigatorForCurrentRoute(observers, restorationScopeId);
...
class StatefulShellRoute extends ShellRouteBase {
  ...
  StatefulShellRoute({
    ...
    GlobalKey<StatefulNavigationShellState>? key,
        ...
        assert((pageBuilder != null) || (builder != null), 'One of builder or pageBuilder must be provided'),
        assert(_debugUniqueNavigatorKeys(branches).length == branches.length, 'Navigator keys must be unique'),
  ...
  @override
  GlobalKey<NavigatorState> navigatorKeyForSubRoute(RouteBase subRoute) {
    final StatefulShellBranch? branch = branches.firstWhereOrNull((StatefulShellBranch e) => e.routes.contains(subRoute));
    assert(branch != null);
    return branch!.navigatorKey;
  }

  Iterable<GlobalKey<NavigatorState>> get _navigatorKeys => branches.map((StatefulShellBranch b) => b.navigatorKey);

  StatefulNavigationShell _createShell(BuildContext context, ShellRouteContext shellRouteContext) => StatefulNavigationShell(
      shellRouteContext: shellRouteContext, router: GoRouter.of(context), containerBuilder: navigatorContainerBuilder);

  static Widget _indexedStackContainerBuilder(BuildContext context, StatefulNavigationShell navigationShell, List<Widget> children) {
    return _IndexedStackedRouteBranchContainer(currentIndex: navigationShell.currentIndex, children: children);
  }

  static List<RouteBase> _routes(List<StatefulShellBranch> branches) => branches.expand((StatefulShellBranch e) => e.routes).toList();

  static Set<GlobalKey<NavigatorState>> _debugUniqueNavigatorKeys(List<StatefulShellBranch> branches) =>
  ...
  static bool _debugValidateParentNavigatorKeys(List<StatefulShellBranch> branches) {
    for (final StatefulShellBranch branch in branches) {
      for (final RouteBase route in branch.routes) {
        if (route is GoRoute) {
          assert(route.parentNavigatorKey == null || route.parentNavigatorKey == branch.navigatorKey);
        }
      }
    }
    return true;
  ...
  static bool _debugValidateRestorationScopeIds(String? restorationScopeId, List<StatefulShellBranch> branches) {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    super.debugFillProperties(properties);
    properties.add(DiagnosticsProperty<Iterable<GlobalKey<NavigatorState>>>('navigatorKeys', _navigatorKeys));
  }
}
...
class StatefulNavigationShell extends StatefulWidget {
  ...
  StatefulNavigationShell({
    ...
    required this.containerBuilder,
        ...
        _router = router,
        currentIndex = _indexOfBranchNavigatorKey(shellRouteContext.route as StatefulShellRoute, shellRouteContext.navigatorKey),
  ...
  void goBranch(int index, {bool initialLocation = false}) {
    final StatefulShellRoute route = shellRouteContext.route as StatefulShellRoute;
  ...
  /// associated with it).
  @visibleForTesting
  List<StatefulShellBranch> get debugLoadedBranches => route._shellStateKey.currentState?._loadedBranches ?? <StatefulShellBranch>[];
  ...
  static StatefulNavigationShellState of(BuildContext context) {
    final StatefulNavigationShellState? shellState = context.findAncestorStateOfType<StatefulNavigationShellState>();
  ...
  static int _indexOfBranchNavigatorKey(StatefulShellRoute route, GlobalKey<NavigatorState> navigatorKey) {
    final int index = route.branches.indexWhere((StatefulShellBranch branch) => branch.navigatorKey == navigatorKey);
    assert(index >= 0);
    return index;
  }
}
...
class StatefulNavigationShellState extends State<StatefulNavigationShell> with RestorationMixin {
  final Map<StatefulShellBranch, _StatefulShellBranchState> _branchState = <StatefulShellBranch, _StatefulShellBranchState>{};
  ...
  String _branchLocationRestorationScopeId(StatefulShellBranch branch) {
    return branch.restorationScopeId != null ? '${branch.restorationScopeId}-location' : identityHashCode(branch).toString();
  }
  ...
  _StatefulShellBranchState _branchStateFor(StatefulShellBranch branch, [bool register = true]) {
    return _branchState.putIfAbsent(branch, () {
      final _StatefulShellBranchState branchState = _StatefulShellBranchState(location: _RestorableRouteMatchList(_router.configuration));
      if (register) {
  ...
  RouteMatchList? _matchListForBranch(int index) => _branchState[route.branches[index]]?.location.value;
  ...
  void _updateCurrentBranchStateFromWidget() {
    ...
    final bool locationChanged = previousBranchLocation != currentBranchLocation;
    if (locationChanged || !hasExistingNavigator) {
      branchState.navigator = shellRouteContext._buildNavigatorForCurrentRoute(branch.observers, branch.restorationScopeId);
    }
  ...
  void _preloadBranches() {
    for (int i = 0; i < route.branches.length; i++) {
      ...
      if (i != currentIndex && branch.preload && !_isBranchLoaded(branch)) {
        ...
        final RouteMatchList matchList = _router.configuration.findMatch(Uri.parse(widget._effectiveInitialBranchLocation(i)));
  ...
  Widget build(BuildContext context) {
    final List<Widget> children = route.branches
        .map((StatefulShellBranch branch) => _BranchNavigatorProxy(
            key: ObjectKey(branch), branch: branch, navigatorForBranch: (StatefulShellBranch branch) => _branchState[branch]?.navigator))
        .toList();
...
class _RestorableRouteMatchList extends RestorableProperty<RouteMatchList> {
  _RestorableRouteMatchList(RouteConfiguration configuration) : _matchListCodec = RouteMatchListCodec(configuration);
  ...
  RouteMatchList fromPrimitives(Object? data) {
    return data == null ? RouteMatchList.empty : _matchListCodec.decode(data as Map<Object?, Object?>);
  }
...
class _IndexedStackedRouteBranchContainer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final List<Widget> stackItems =
        children.mapIndexed((int index, Widget child) => _buildRouteBranchContainer(context, currentIndex == index, child)).toList();
